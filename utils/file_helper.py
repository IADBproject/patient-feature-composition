...
class FileHelper(object):
    ...
    @staticmethod
    def save_arraytotext(filename, voc_x2_name):
        """
            Save voc to text using numpy np.savetxt
        """
        with open(filename, 'w') as file:
            np.savetxt(file, voc_x2_name, delimiter=',', fmt='%s')

    @staticmethod
    def save_text(filename, voc_x2_name):
        """
            Save voc to text using numpy np.savetxt
        """
        with open(filename, 'w') as file:
            file.write(voc_x2_name)
            #np.savetxt(file, voc_x2_name, delimiter=',', fmt='%s')
